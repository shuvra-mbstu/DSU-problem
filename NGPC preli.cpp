...
void distance()
{
    ...
    FOR(i,k+1,num)
    {
        ...
        int x=(vt[k].first-vt[i].first);
        int y=(vt[k].second-vt[i].second);
...
res[num]=0.0;
    FOR(k,0,j)
    {
        int a=Find(ss[k].nod1);
        int b=Find(ss[k].nod2);
//cout<<a<<" "<<b<<" "<<ss[k].dis<<endl;
        if(a!=b)
        {
            if(mm[b]>mm[a])
                swap(a,b);
